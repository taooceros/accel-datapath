...
OUTPUT_DIR=${DSA_FFI_VERIFY_OUTPUT_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/dsa-ffi-await-memmove.XXXXXX")}
...
ARTIFACT_PATH="${OUTPUT_DIR}/await_memmove.json"
STDOUT_PATH="${OUTPUT_DIR}/await_memmove.stdout"
STDERR_PATH="${OUTPUT_DIR}/await_memmove.stderr"
...
BINARY_PATH=${DSA_FFI_VERIFY_BINARY:-${ACCEL_RPC_DIR}/target/${TARGET_SUBDIR}/await_memmove}
...
fail_phase() {
  local phase=$1
  shift
  printf '[verify_async_memmove] phase=%s output_dir=%s artifact=%s %s\n' "${phase}" "${OUTPUT_DIR}" "${ARTIFACT_PATH}" "$*" >&2
  exit 1
}
...
if [[ -n "${DSA_FFI_VERIFY_BINARY:-}" && "${SKIP_BUILD}" != "1" ]]; then
  fail_phase preflight "device_path=${DEVICE_PATH} launcher_status=contradictory_overrides message=DSA_FFI_VERIFY_BINARY requires DSA_FFI_VERIFY_SKIP_BUILD=1 so the verifier does not build one binary and execute another"
fi
...
if [[ "${SKIP_BUILD}" != "1" ]]; then
  ...
  (
    cd "${ACCEL_RPC_DIR}"
    cargo build --profile "${BUILD_PROFILE}" -p dsa-ffi --bin await_memmove
  )
fi
...
if ok:
    ...
    if error_kind not in {'worker_failure', 'validation_failure'}:
        raise SystemExit('failed artifact error_kind must be worker_failure or validation_failure')
