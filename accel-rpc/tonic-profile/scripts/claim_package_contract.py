...
def validate_scope(scope_obj: Any) -> Dict[str, Any]:
    ...
    if validated_pairing_keys != REQUIRED_PAIRING_KEYS:
        raise ManifestError(
            "manifest.scope.pairing_keys must be exactly: " + ", ".join(REQUIRED_PAIRING_KEYS)
        )
...
def validate_report(report_obj: Any, run_root: str) -> Dict[str, Any]:
    ...
    if path != EXPECTED_REPORT_PATH:
        raise ManifestError(f"manifest.report.path={path!r} expected {EXPECTED_REPORT_PATH!r}")
...
def validate_derived_outputs(derived_outputs_obj: Any) -> Dict[str, str]:
    ...
    for key, expected in EXPECTED_DERIVED_OUTPUTS.items():
        ...
        if actual != expected:
            raise ManifestError(f"manifest.derived_outputs.{key}={actual!r} expected {expected!r}")
        if actual in seen_paths:
            raise ManifestError(f"manifest.derived_outputs.{key} duplicates output path {actual!r}")
...
def validate_artifact_families(
    ...
    software_manifest = load_upstream_manifest(inputs["software_manifest"], scope="inputs.software_manifest")
...
def summary_output_paths(manifest: Dict[str, Any], run_root: Path) -> Dict[str, Path]:
    return {key: run_root / relpath for key, relpath in manifest["derived_outputs"].items()}
