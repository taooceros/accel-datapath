...
// Benchmark DSA dynamic batch with inline polling, returns bandwidth and page faults
template <typename DsaType>
DsaMetric benchmark_dynamic_inline(DsaType &dsa, size_t batch_size, size_t msg_size,
                                   std::vector<char> &src, std::vector<char> &dst,
                                   int iterations) {
  ...
  double bw = (double)total_size * iterations / (1024.0 * 1024.0 * 1024.0) / diff.count();
  return {bw, page_faults};
}

// Benchmark DSA dynamic batch with background thread polling, returns bandwidth and page faults
template <typename DsaType>
DsaMetric benchmark_dynamic_threaded(DsaType &dsa, size_t batch_size, size_t msg_size,
                                     std::vector<char> &src, std::vector<char> &dst,
                                     int iterations) {
  size_t total_size = batch_size * msg_size;
...
std::string format_metric(const DsaMetric &m) {
  return fmt::format("{:.2f}GB/s({})", m.bandwidth, m.page_faults);
}
...
void benchmark_queues_with_dsa() {
  ...
  for (auto bs : batch_sizes) {
    for (auto ms : msg_sizes) {
      ...
      { DsaTasSpinlock dsa(false); result.tas_spinlock = benchmark_dynamic_inline(dsa, bs, ms, src, dst, iterations); }
