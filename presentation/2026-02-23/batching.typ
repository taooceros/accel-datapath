...
#figure(
  table(
    ...
    table.header[*Strategy*][*Submitter class*][*Doorbells/$N$*][*In-flight*][*Blocks?*][*Key idea*],
    [Immediate],       [`DirectSubmitter`],        [$N$],           [N/A],  [No],  [1 doorbell per descriptor],
    [Double-Buffered], [`StagingSubmitter`],        [$ceil(N\/B)$],  [2],    [Yes], [Two fixed arrays, swap on submit],
    [Fixed-Ring],      [`FixedRingSubmitter`],      [$ceil(N\/B)$],  [16],   [No],  [Ring of fixed-size batch entries],
    [Ring-Buffer],     [`RingSubmitter`],           [$ceil(N\/B)$],  [16],   [No],  [Shared descriptor ring + batch metadata ring],
    [Mirrored-Ring],   [`MirroredRingSubmitter`],   [$ceil(N\/B)$],  [16],   [No],  [VM-aliased ring --- no wrap-around logic],
  ),
  caption: [Strategy overview. $N$ = operations, $B$ = max batch size (typically 32). All submitters satisfy the `DescriptorSubmitter` concept.]
) <overview-fig>

#keypoint[
  *Key result:* Ring-buffer and mirrored-ring batching achieve *1.2--2.0x* higher message rates than double-buffered batching across all 8 DSA operations, by eliminating submission blocking and improving batch utilization. Fixed-ring batching (ablation study) confirms that most of the gain comes from deeper in-flight capacity (16 vs. 2 batches), not memory packing. The mirrored-ring variant further simplifies the implementation by eliminating wrap-around handling via virtual memory mirroring.
]

All five strategies present the same interface through `DsaProxy` type erasure (via Microsoft's proxy library) --- scheduling strategies work identically across all backends. Each submitter is composed with `DsaEngine<Submitter, QueueTemplate>` to produce concrete DSA types (e.g. `DsaBatch = DsaEngine<StagingSubmitter, MutexTaskQueue>`).
...
== Double-Buffered (`StagingSubmitter`) <double-buffered>
...
== Fixed-Ring (`FixedRingSubmitter`) <fixed-ring>
...
== Ring-Buffer (`RingSubmitter`) <ring-buffer>

#keypoint[
  *Idea:* Two separate ring buffers --- a descriptor ring (512 slots at default sizing, dynamically computed as `round_up_pow2(max_batch_size_ * num_batches_)`) for tightly-packed storage and a batch metadata ring (16 entries) for tracking in-flight batches. Full batches auto-submit; partial batches drain on `poll()`. 16 in-flight batches eliminate blocking.
]
...
// ──────────────────────────────────────────────────────────────
== Mirrored-Ring (`MirroredRingSubmitter`) <mirrored-ring>

#keypoint[
  *Idea:* Same two-ring architecture as `RingSubmitter` (shared descriptor ring + batch metadata ring) but the descriptor ring is backed by a `MirroredRing`: the same physical pages are mapped twice contiguously in virtual memory via `memfd_create` + two `MAP_FIXED` mappings. This eliminates the wrap-around check entirely --- a batch that starts near the end of the ring sees contiguous memory via the mirror region.
]

*How it works:*
- Identical to `RingSubmitter` except: no wrap-around check in `submit_descriptor()`. When a batch spans the ring boundary (e.g. start=480, count=32 with ring size 512), slots 480..511 and the mirrored copies at virtual addresses 0..31 are contiguous in the process's virtual address space.
- Same `pre_poll()` / `drain()` / `reclaim_completed()` logic as `RingSubmitter`.
- `MirroredRing` RAII wrapper handles `memfd_create`, `ftruncate`, and double-mapping cleanup.

*Advantages over `RingSubmitter`:*
- *No early batch seals* --- batches always fill to `max_batch_size_` (better utilization)
- *Simpler hot path* --- no conditional branch for wrap-around
- *Same memory footprint* --- identical ring sizes

*Trade-offs:*
- Requires `memfd_create` and `MAP_FIXED` (Linux-specific)
- Page-aligned allocation (ring size rounded up to page boundary)
...
Average latency drops 30--46% for lightweight ops. The p99 for copy\_crc and cache\_flush shows slight regression with `RingSubmitter`, likely from occasional wrap-around batch splits. The `MirroredRingSubmitter` eliminates this by removing wrap-around handling entirely.
...
#keypoint[
  ...
  The mirrored-ring variant (@mirrored-ring) further improves on ring-buffer by eliminating early batch seals at the ring boundary, ensuring consistently full batches.
]
...
#figure(
  table(
    ...
    inset: 6pt,
    fill: (_, y) => if y == 0 { rgb("#f8fafc") },
    table.header[*Property*][*Immediate*][*Double-Buffered*][*Fixed-Ring*][*Ring-Buffer*][*Mirrored-Ring*],
    [Doorbells per $N$ ops], [$N$], [$ceil(N \/ B)$], [$ceil(N \/ B)$], [$ceil(N \/ B)$], [$ceil(N \/ B)$],
    [Staging buffers], [None], [2 fixed arrays], [16 fixed arrays], [1 shared ring], [1 mirrored ring],
    [Max in-flight batches], [N/A], [2], [16], [16], [16],
    [Submit blocks?], [No], [Yes (on prev batch)], [No], [No], [No],
    [Descriptor waste], [None], [Up to $2 times 32$ slots], [Up to $16 times 32$ slots], [Only at wrap], [None],
    [Memory footprint], [0], [$approx$4 KB], [$approx$34 KB], [$approx$33 KB], [$approx$33 KB],
    [Wrap-around handling], [N/A], [N/A], [N/A], [Early batch seal], [None (VM mirror)],
    [Submitter class], [`DirectSubmitter`], [`StagingSubmitter`], [`FixedRingSubmitter`], [`RingSubmitter`], [`MirroredRingSubmitter`],
  ),
...
#figure(
  table(
    ...
    [`kDefaultNumBatches`], [16], [16 concurrent in-flight batches],
    [`kDefaultBatchSize`], [32], [Default max descriptors per batch],
    [`desc_ring_size_`], [`round_up_pow2(32 #sym.times 16)` = 512], [Dynamically computed; power-of-2 for masking],
    [`max_batch_size_`], [min(hw, 32)], [Per-batch limit from hardware],
    [Total memory], [$approx$33 KB], [512 #sym.times 64 B desc ring + 16 #sym.times batch metadata],
  ),
  caption: [Ring-buffer sizing parameters. Ring size is computed at `init()` time, not a compile-time constant.]
)
...
#figure(
  table(
    ...
    fill: (_, y) => if y == 0 { rgb("#f8fafc") },
    table.header[*Parameter*][*Value*][*Rationale*],
    [`kDefaultNumBatches`], [16], [Same depth as ring-buffer],
    [`batch_capacity_`], [32], [Per-entry descriptor array size],
    [`max_batch_size_`], [min(hw, 32)], [Per-batch limit from hardware],
...
All five submitters satisfy the `DescriptorSubmitter` concept and are composed with `DsaEngine<Submitter, QueueTemplate>` to produce concrete DSA types. The `DsaFacade` interface (via Microsoft's proxy library) provides type erasure through `DsaProxy`, enabling runtime strategy selection. All batch submitters (`StagingSubmitter`, `FixedRingSubmitter`, `RingSubmitter`, `MirroredRingSubmitter`) use composition --- each contains a `DirectSubmitter direct_` member for raw MMIO submission, rather than using inheritance.

== Template Parameterization

`DsaEngine` is templated on both submitter type and queue type (`SingleThreadTaskQueue`, `MutexTaskQueue`, `TasSpinlockTaskQueue`, `SpinlockTaskQueue`, `BackoffSpinlockTaskQueue`, `LockFreeTaskQueue`, `IndexedSingleThreadTaskQueue`) with explicit instantiations in separate `.cpp` files (e.g. `dsa_staging_instantiate.cpp`, `dsa_ring_instantiate.cpp`, `dsa_mirrored_ring_instantiate.cpp`).
