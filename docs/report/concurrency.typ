...
The dsa-stdexec framework implements nine scheduling patterns organised into three families for dispatching operations to Intel DSA hardware. Each pattern makes different trade-offs in allocation cost, stdexec overhead, concurrency control, and programming model.
...
#figure(
  table(
    ...
    table.header[*Family*][*Pattern*][*Allocs/op*][*Acquire*][*Overlap*][*Key idea*],
    table.cell(rowspan: 5)[Sliding\ Window], [sliding\_window], [1 (heap)], [---], [Full], [Atomic `in_flight` counter],
    [noalloc],        [0],         [$O(C)$ scan],  [Full],  [Pre-sized slots + `SlotReceiver`],
    [arena],          [0],         [$O(1)$ pop],   [Full],  [Intrusive free-list + `ArenaReceiver`],
    [direct],         [0],         [$O(1)$ pop],   [Full],  [Bypass `scope.nest`/`then`; `DirectBenchReceiver`],
    [reusable],       [0],         [$O(1)$ pop],   [Full],  [Bypass stdexec entirely; reuse `DsaOperationBase`],
    table.cell(rowspan: 3)[Batch], [heap\_alloc], [1 (heap)], [---], [None], [Barrier: submit $N$, wait all, repeat],
    [noalloc],        [0],         [$O(C)$ scan],  [None],  [Pre-sized slots per batch],
    [raw],            [0],         [---],          [None],  [Hardware batch descriptor via `dsa_batch` sender],
    [Scoped\ Workers], [scoped\_workers], [$approx$0], [---], [Full], [$N$ coroutines with `co_await`],
  ),
  caption: [Strategy overview. $C$ = concurrency level. "Overlap" indicates whether new operations can start before previous ones complete. `direct` and `reusable` are inline-only; `raw` is inline-only.]
) <overview-fig>

All nine patterns share the same function signature via `StrategyParams` and are dispatched through a $9 times 2$ strategy table (9 patterns $times$ 2 polling modes). Some entries are `nullptr` for patterns that only support inline polling.
...
The slot size is computed at compile time by tracing the full sender type chain. Inline and threaded polling require different computations because threaded mode wraps in `schedule() | let_value(...)`, producing a much larger operation state:
...
constexpr size_t inline_noalloc_slot_size() {
  ...
  using ThenSender = decltype(Sender{} | stdexec::then(CompletionRecord{}));
  using NestSender = exec::async_scope::nest_result_t<ThenSender>;
  return sizeof(stdexec::connect_result_t<NestSender, SlotReceiver>);
}

// Threaded slot size (includes scheduler wrapping — ~768 B vs ~448 B)
template <class MakeSender>
...
- Different slot sizes for inline vs. threaded (~448 B vs ~768 B due to scheduler wrapping)
...
// ──────────────────────────────────────────────────────────────
== Sliding Window: Direct <direct>

#keypoint[
  *Idea:* Bypass `async_scope::nest()` and `stdexec::then()` entirely. Connect the raw DSA sender directly to a custom `DirectBenchReceiver` that combines arena-style slot recycling, latency recording, and remaining-op counting in a single receiver. Inline polling only.
]

The slot size is computed differently from noalloc/arena — no nest or then wrapper:
...
*Trade-offs:*
- Removes ~14 ns/op by eliminating scope.nest and then-adapter overhead
- Still uses stdexec `connect()` and `start()` per operation
- Inline polling only (no threaded mode)
- Measures the cost of stdexec's composability wrappers in isolation

// ──────────────────────────────────────────────────────────────
== Sliding Window: Reusable <reusable>

#keypoint[
  *Idea:* Bypass stdexec `connect()` and `start()` entirely. Pre-allocate `DsaOperationBase` storage and reuse it across operations. The hot path is just `memset → fill_descriptor → submit`. No page-fault retry. Inline polling only.
]

This is the lowest-overhead strategy (~8 ns/op on mock DSA) and serves as the hardware throughput reference. It directly manipulates `DsaOperationBase` storage without going through the stdexec sender/receiver machinery.

*Trade-offs:*
- Absolute minimum software overhead — isolates raw hardware throughput
- No stdexec in the hot path at all (no connect, no start, no receivers)
- No page-fault retry (operations that page-fault will fail)
- Not composable — purely a benchmark reference point
...
// ──────────────────────────────────────────────────────────────
== Batch: No-Alloc <batch-noalloc>

#keypoint[
  *Idea:* Same barrier-based batch scheduling as the heap-alloc batch strategy, but uses pre-allocated `OperationSlot[]` per batch with placement-new. Eliminates per-op heap allocation while retaining the batch synchronization pattern. Supports both inline and threaded polling.
]

*Trade-offs:*
- Zero heap allocations in the hot path (same as sliding-window noalloc)
- Still no pipeline overlap (barrier between batches)
- Useful baseline for measuring allocation overhead in batch workloads

// ──────────────────────────────────────────────────────────────
== Batch: Raw <batch-raw>

#keypoint[
  *Idea:* Use the hardware batch descriptor sender (`dsa_batch` from `batch.hpp`) to submit a group of descriptors as a single hardware batch command. Bypasses `async_scope` and per-op latency recording. Inline polling only.
]

This strategy constructs individual descriptors via `fill_for_op()` and submits them together through the `dsa_batch()` sender, which maps directly to DSA's batch opcode (`0x01`). It is the reference point for measuring hardware batch submission overhead.

*Trade-offs:*
- Minimal software overhead for batch submission
- Inline polling only
- No per-op latency recording
- Measures the `dsa_batch` sender path, complementing the submission-level batching in `descriptor_submitter.hpp`
...
All sliding-window variants (standard, no-alloc, arena, direct, reusable) and scoped workers achieve full pipeline overlap. The batch strategies (heap\_alloc, noalloc, raw) have zero overlap --- they trade throughput for simplicity.
...
#figure(
  cetz.canvas(length: 1cm, {
    ...
    // Scheduling column — 3 families with sub-strategies
    let sx = 0
    content((sx + 1.8, 6.3), text(size: 8pt, weight: "bold")[Scheduling (9 patterns)])
    let strats = ("SW: sliding_window", "SW: noalloc", "SW: arena", "SW: direct", "SW: reusable", "Batch: heap_alloc", "Batch: noalloc", "Batch: raw", "Scoped Workers")
    for (i, name) in strats.enumerate() {
      let y = 5.6 - i * 0.55
      rect((sx, y - 0.2), (sx + 3.5, y + 0.2), fill: col-submit.lighten(75%), stroke: 0.4pt, radius: 3pt)
      content((sx + 1.75, y), text(size: 6pt)[#name])
    }

    // DsaProxy
    let mx = 5.0
    rect((mx, 0.55), (mx + 2.0, 6.0), fill: luma(242), stroke: (paint: luma(140), thickness: 1pt), radius: 4pt)
    content((mx + 1.0, 5.7), text(size: 7.5pt, weight: "bold")[DsaProxy])
    content((mx + 1.0, 5.1), text(size: 6pt, fill: luma(100))[type erasure])
    for (i, m) in ("submit()", "poll()", "flush()").enumerate() {
      content((mx + 1.0, 4.2 - i * 0.5), text(size: 6.5pt, font: "DejaVu Sans Mono")[#m])
    }

    // Submission column — 5 backends
    let rx = 8.8
    content((rx + 1.6, 6.3), text(size: 8pt, weight: "bold")[Submission (5 backends)])
    let backs = ("Immediate", "Double-Buffered", "Fixed-Ring", "Ring-Buffer", "Mirrored-Ring")
    let bcols = (col-done, col-hw, col-poll, col-coro, col-submit)
    for (i, name) in backs.enumerate() {
      let y = 5.3 - i * 0.7
      rect((rx, y - 0.22), (rx + 2.8, y + 0.22), fill: bcols.at(i).lighten(75%), stroke: 0.4pt, radius: 3pt)
      content((rx + 1.4, y), text(size: 6.5pt)[#name])
    }

    // Arrows
    for i in range(9) {
      let y = 5.6 - i * 0.55
      line((sx + 3.5, y), (mx, calc.clamp(y, 0.7, 5.8)), stroke: (paint: luma(190), thickness: 0.35pt), mark: (end: ">"))
    }
    for i in range(5) {
      let y = 5.3 - i * 0.7
      line((mx + 2.0, calc.clamp(y, 0.7, 5.8)), (rx, y), stroke: (paint: luma(190), thickness: 0.35pt), mark: (end: ">"))
    }
  }),
