...
- **Paper folder**: [`papers/hermes-enhancing-layer-7-cloud-load-balancers-with-userspace-directed-i-o-event-notification/README.md`](papers/hermes-enhancing-layer-7-cloud-load-balancers-with-userspace-directed-i-o-event-notification/README.md)
- **Source URL**: https://ng-95.github.io/files/Hermes_SIGCOMM25.pdf

### Repo summary

Hermes turns userspace worker state into an explicit control input for kernel connection dispatch, combining shared worker metrics with an eBPF override of `SO_REUSEPORT` selection. For this repo, it is one of the clearest recent papers showing that event-notification and completion policy remain first-class bottlenecks once the hottest work is no longer just packet movement in the kernel.

### Technique

- **Userspace-directed I/O event notification**: close the loop between userspace worker state and kernel connection dispatch.
- **Worker Status Table (WST)**: track event-loop timestamps, pending-event counts, and accumulated connection counts in shared memory.
- **Worker-triggered distributed scheduling**: filter candidate workers in userspace rather than asking the kernel to infer load from packet-level state alone.
- **eBPF-based `reuseport` override**: use eBPF maps and `SO_ATTACH_REUSEPORT_EBPF` to execute the final constrained dispatch decision in the kernel.

### Why it matters here

This paper strengthens the repo's interest in treating completion and dispatch policy as its own design layer. Hermes is especially relevant when arguing that a fast path can stay narrow while richer load-awareness and scheduling logic live in a colder control loop without giving up deployability.

### Gap relative to this repo

Hermes is about Linux L7 load balancing, not on-die accelerators or Rust async composition. Its value here is as a control-path and event-notification analogue, not as a direct implementation template for the repo's DSA or `accel-rpc` stack.

### RQ tags

- **RQ2**: strong support for redesigning completion and event-notification policy around lighter hot paths
- **RQ3**: medium relevance to end-to-end RPC-adjacent control-path decomposition under diverse workloads
- **RQ4**: medium support that software scheduling bottlenecks generalize beyond the DSA setting

### Extracted opening text

> “In this work, we present Hermes, a userspace-directed I/O event notification framework to enhance L7 LBs.”

> “Hermes uses userspace worker status to direct kernel-space connection dispatch.”

> “Hermes has been deployed on O(100K) CPU cores in Alibaba Cloud, handling O(10M) RPS of traffic. It reduces daily worker hangs by 99.8% and lowers the unit cost of L7 LB infrastructure by 18.9%.”
